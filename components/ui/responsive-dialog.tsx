  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogHeader
          className={cn(
            "relative shrink-0 border-b border-border/50 bg-background/95 px-4 pb-3 pt-2 text-left backdrop-blur supports-[backdrop-filter]:bg-background/80 sm:px-6 sm:py-4",
            headerClassName
          )}
        >
          ...
          <div
            className={cn(
              "grid gap-3",
              headerAccessory && "md:grid-cols-[minmax(0,1fr)_auto] md:items-center",
          ...
          {showCloseButton && (
            <DialogClose
              ...
              className="absolute right-4 top-2 inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-muted-foreground transition-colors hover:bg-accent/70 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/50 disabled:pointer-events-none disabled:opacity-50 sm:right-6 sm:top-4"
            >
              <X className="h-4 w-4" />
