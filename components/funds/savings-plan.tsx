  ...
  return (
    <div className="space-y-2">
      <div className="flex items-baseline justify-between gap-3">
        <span className="font-semibold tracking-tight">{formatCurrency(value)} <span className="font-normal text-muted-foreground">of {formatCurrency(target)}</span></span>
...
function PaceHint({ item, onUse }: { item: SavingsPlanFundItem; onUse?: () => void }) {
  ...
  if (pace.status === "on_track_calculable" && pace.recommended_amount) {
    return <p className="text-sm text-muted-foreground">To stay on pace: {formatCurrency(pace.recommended_amount)} this month {onUse ? <button type="button" className="ml-1 font-medium text-foreground underline underline-offset-4" onClick={onUse}>Use amount</button> : null}</p>
  }
  if (pace.status === "goal_met") return <p className="text-sm text-muted-foreground">Goal reached</p>
  if (pace.status === "overdue" && pace.goal_shortfall) return <p className="text-sm text-muted-foreground">Target was earlier · {formatCurrency(pace.goal_shortfall)} remaining</p>
  return null
}

function FundPlanRow({ item, editing, value, onChange, onRemove, onUse }: { item: SavingsPlanFundItem; editing?: boolean; value?: string; onChange?: (value: string) => void; onRemove?: () => void; onUse?: () => void }) {
  const progress = amount(item.progress_amount)
  ...
  return (
    <div className="space-y-2 border-b border-border/70 py-5 last:border-b-0">
      <div className="flex items-start justify-between gap-3">
        ...
        ) : <span className="font-semibold">{formatCurrency(item.planned_amount)}</span>}
      </div>
      {!editing ? <>
        <MoneyProgress value={item.progress_amount} target={item.planned_amount} label={amount(item.over_plan_amount) > 0 ? `${formatCurrency(item.over_plan_amount)} over plan` : amount(item.remaining_planned) > 0 ? `${formatCurrency(item.remaining_planned)} remaining` : "Plan complete"} />
        <PaceHint item={item} onUse={onUse} />
        {amount(item.closeout_contributed) > 0 ? <p className="text-xs text-muted-foreground">{formatCurrency(item.transaction_contributed)} saved during the month · {formatCurrency(item.closeout_contributed)} from month closeout</p> : null}
      </> : null}
    </div>
  )
}
  ...
  return (
    <section className="space-y-3 border-y border-border/70 py-5">
      ...
      {!budget ? <><p className="font-semibold">Set {formatMonthLabel(month) ?? month}’s budget before creating a Savings Plan.</p><Link className="text-sm font-medium underline underline-offset-4" href="/settings/budget">Set monthly budget →</Link></> : <>
        <MoneyProgress value={data.summary.saved_amount} target={budget} label={data.summary.over_saved_amount !== "0.00" ? `${formatCurrency(data.summary.over_saved_amount)} above plan` : `${formatCurrency(data.summary.remaining_to_save)} left to save`} />
        <div className="flex flex-wrap gap-x-5 gap-y-1 text-sm text-muted-foreground"><span>{formatCurrency(data.summary.planned_to_funds)} planned for Funds</span><span>{formatCurrency(data.summary.unassigned_budget)} not assigned</span></div>
...
export function SavingsPlanFundContext({ fundId, month = getCurrentMonthKey() }: { fundId: string; month?: string }) {
  ...
  return <section className="space-y-3 border-y border-border/70 py-5"><p className="text-xs font-medium uppercase tracking-[0.24em] text-muted-foreground">This month</p>{amount(item.planned_amount) > 0 ? <MoneyProgress value={item.progress_amount} target={item.planned_amount} label={amount(item.remaining_planned) > 0 ? `${formatCurrency(item.remaining_planned)} remaining` : "Plan complete"} /> : <p className="font-medium">No amount planned</p>}<PaceHint item={item} /><Link className="inline-block text-sm font-medium underline underline-offset-4" href={`/funds/savings-plan?month=${month}`}>View Savings Plan →</Link></section>
}
...
export function SavingsPlanPage({ initialMonth }: { initialMonth?: string }) {
  ...
  return <div className="min-h-screen bg-background pb-mobile-nav"><main className="mx-auto max-w-lg px-5 pb-10 pt-standalone-safe-top lg:max-w-3xl lg:px-8">
    <div className="space-y-6">
      ...
      <div className="space-y-3"><p className="text-xs font-medium uppercase tracking-[0.24em] text-muted-foreground">Savings Plan</p><h1 className="text-3xl font-semibold tracking-tight">{formatMonthLabel(month) ?? month}</h1><MonthSelector currentMonth={month} onChange={(next) => { setEditing(false); setMonth(next) }} allowFuture /></div>
      {isLoading ? <div className="space-y-4 py-10"><Spinner className="size-5" /><div className="h-8 w-48 animate-pulse rounded bg-muted" /><div className="h-20 animate-pulse rounded bg-muted" /></div> : error || !data ? <div className="space-y-3 border-y border-border/70 py-6"><p className="text-sm text-destructive">{error ?? "Unable to load Savings Plan"}</p><Button variant="outline" onClick={() => void refetch()}>Retry</Button></div> : !data.budget.has_budget ? <div className="space-y-3 border-y border-border/70 py-6"><p className="text-xl font-semibold">{formatMonthLabel(month) ?? month} doesn’t have a budget yet.</p><p className="text-sm text-muted-foreground">Set your monthly Savings amount first, then decide where you want it to go.</p><Button asChild><Link href="/settings/budget">Set monthly budget →</Link></Button></div> : editing ? <>
        ...
        <section className="space-y-5 border-y border-border/70 py-5"><MoneyProgress value={data.summary.saved_amount} target={data.budget.savings_budget ?? "0"} label={data.summary.over_saved_amount !== "0.00" ? `${formatCurrency(data.summary.over_saved_amount)} above plan` : `${formatCurrency(data.summary.remaining_to_save)} left to save`} />{data.summary.is_overallocated ? <p className="text-sm text-muted-foreground">Plan needs review · {formatCurrency(data.summary.overallocation_amount)} over budget</p> : null}</section>
        {data.goal_pacing.status === "available" && data.goal_pacing.recommended_total && data.goal_pacing.gap_to_savings_budget && amount(data.goal_pacing.gap_to_savings_budget) > 0 ? <p className="border-b border-border/70 pb-5 text-sm text-muted-foreground">Your target-date goals would need about {formatCurrency(data.goal_pacing.recommended_total)} this month to stay on pace. That’s {formatCurrency(data.goal_pacing.gap_to_savings_budget)} more than your Savings budget.</p> : null}
        <section><p className="text-xs font-medium uppercase tracking-[0.24em] text-muted-foreground">Your plan</p>{data.funds.filter((item) => amount(item.planned_amount) > 0).map((item) => <FundPlanRow key={item.fund.id} item={item} />)}{data.funds.filter((item) => amount(item.planned_amount) === 0 && amount(item.progress_amount) > 0).length > 0 ? <div className="mt-6 border-t border-border/70 pt-5"><p className="text-xs font-medium uppercase tracking-[0.24em] text-muted-foreground">Other Fund activity</p>{data.funds.filter((item) => amount(item.planned_amount) === 0 && amount(item.progress_amount) > 0).map((item) => <div key={item.fund.id} className="flex justify-between border-b border-border/70 py-4 text-sm"><span>{item.fund.name}<span className="block text-muted-foreground">No amount planned</span></span><span className="font-semibold">{formatCurrency(item.progress_amount)} directed</span></div>)}</div> : null}</section>
        <div className="flex flex-wrap gap-x-5 gap-y-2 border-t border-border/70 pt-5 text-sm text-muted-foreground"><span>{formatCurrency(data.summary.planned_to_funds)} planned for Funds</span><span>{formatCurrency(data.summary.unassigned_budget)} not assigned</span></div>
        {data.status === "closed" ? <p className="text-sm text-muted-foreground">Closed</p> : !data.has_plan ? <div className="space-y-2 border-t border-border/70 pt-5"><p className="font-semibold">{formatCurrency(data.budget.savings_budget ?? "0")} planned for Savings</p><p className="text-sm text-muted-foreground">Decide how much you want to direct toward your Funds this month.</p><Button className="rounded-full" onClick={startEditing}>Create plan</Button></div> : null}
      </>}
    </div>
  </main></div>
}
