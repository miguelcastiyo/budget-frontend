...
function FundPlanRow({ item, editing, value, onChange, onRemove }: { item: SavingsPlanFundItem; editing?: boolean; value?: string; onChange?: (value: string) => void; onRemove?: () => void }) {
  return (
    <div className="space-y-2 border-b border-border/70 py-5 last:border-b-0">
      ...
      {!editing ? <div className="flex items-center justify-between gap-4 text-sm text-muted-foreground"><span>{formatCurrency(item.planned_amount)} planned</span><span className="shrink-0 font-medium text-foreground">{formatCurrency(item.progress_amount)} directed</span></div> : null}
    </div>
  )
}
