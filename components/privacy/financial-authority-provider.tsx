...
import type { CreateTransactionRequest, CreateFundEntryRequest, FundDetail, FundEntriesPage, FundEntry, FundListItem, FundsListResponse, Transaction, UpdateFundEntryRequest, UpdateTransactionRequest } from "@/lib/api/types"
...
interface FinancialAuthorityContextValue {
  ...
  quickUnlockUnavailableReason: "ios_standalone" | null
...
export function FinancialAuthorityProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateEncryptedTransaction = async (current: Transaction, input: UpdateTransactionRequest) => {
    ...
    await authority!.commitSourceDiff(transactionFundDiff(transactionFundState({ id: current.id, family: "transaction", data: currentRecord.data }), transactionFundState({ id: current.id, family: "transaction", data: nextData })))
    const saved = authority!.store.get(current.id)!
    return uiTransaction(saved)
  }
  const deleteEncryptedTransaction = async (current: Transaction) => { encryptedUserId(); const existing = authority!.store.get(current.id); if (!existing) throw new Error("ENCRYPTED_RECORD_NOT_FOUND"); const ledger = authority!.getState().fundLedgerEntries.find((item) => String(item.source_transaction_id ?? "") === current.id); const priorEntry: SourceRecord | null = ledger ? { id: String(ledger.id), family: "fund_ledger_entry", data: ledger } : null; await authority!.commitSourceDiff(transactionFundDiff(transactionFundState({ id: current.id, family: "transaction", data: existing.data }, priorEntry), transactionFundState({ id: current.id, family: "transaction", data: { ...existing.data, is_deleted: true } }, null))) }
  ...
  const getEncryptedFund = async (fundId: string): Promise<FundDetail> => { const list = await getEncryptedFunds({ status: "all" }); const item = list.items.find((fund) => fund.id === fundId); if (!item) throw new Error("FUND_NOT_FOUND"); const entries = authority!.getState().fundLedgerEntries.filter((raw) => String(raw.fund_id ?? "") === fundId).map((raw) => encryptedFundEntry(raw, encryptedFundEntrySourceId(raw), fundId)); return { ...item, source_breakdown: { month_closeout: "0.00", transaction: "0.00", manual: "0.00", starting_balance: "0.00", correction: "0.00" }, entries_count: entries.length, recent_entries: entries } }
