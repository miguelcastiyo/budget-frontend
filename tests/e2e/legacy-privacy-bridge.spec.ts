...
type SeededAccount = { email: string; password: string; session_token: string; csrf_token: string; canary: string }
...
test("legacy account can complete Privacy & Vault setup from Settings", async ({ page }, testInfo) => {
  ...
  await page.getByLabel("Vault passphrase", { exact: true }).fill("local bridge passphrase")
  await page.getByLabel("Confirm Vault passphrase", { exact: true }).fill("local bridge passphrase")
  ...
  await page.getByRole("button", { name: "Continue", exact: true }).click()
  await page.getByTestId("privacy-setup-encrypt").click()
  await expect(page.getByTestId("privacy-migration-progress")).toBeVisible()
  await expect(page.getByRole("heading", { name: "You're ready", exact: true })).toBeVisible({ timeout: 60_000 })
  ...
  await page.goto("/")
  await expect(page.getByTestId("encrypted-vault-locked-boundary")).toBeVisible()
  await page.goto("/settings/vault")
  await page.getByRole("button", { name: /Vault.*Locked/ }).click()
  await page.getByLabel("Vault passphrase", { exact: true }).fill("local bridge passphrase")
  await page.getByRole("button", { name: "Unlock Vault", exact: true }).click()
  await expect(page.getByRole("alert")).toContainText("Vault unlocked.")
})
