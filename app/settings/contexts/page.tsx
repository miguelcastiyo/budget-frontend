...
import { getContextIcon } from "@/lib/tag-icons"
...
export default function ContextsSettingsPage() {
  ...
  return (
    <div className="min-h-screen bg-background pb-mobile-nav">
      ...
      <ResponsiveDialog open={showNew} onOpenChange={(open) => open ? setShowNew(true) : resetNew()} title="Create context" description="Choose a name and icon." mobileSize="compact" footer={<div className="grid grid-cols-[auto_minmax(0,1fr)] gap-2"><Button type="button" variant="ghost" className="h-12 rounded-xl" onClick={resetNew}>Cancel</Button><Button type="button" className="h-12 rounded-xl" disabled={!newName.trim() || isMutating} onClick={() => void createContext()}>{isMutating ? "Creating..." : "Create context"}</Button></div>}>
        <div className="grid gap-4"><div className="relative"><NewIcon className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input value={newName} onChange={(event) => setNewName(event.target.value)} placeholder="Context name" className="h-12 rounded-xl pl-10" /></div><TagIconPicker tagName={newName} value={newIconKey} onChange={setNewIconKey} entityLabel="Context" /></div>
      </ResponsiveDialog>
      <ResponsiveDialog open={editingId !== null} onOpenChange={(open) => !open && cancelEdit()} title="Edit context" description={editing?.name ?? "Update name and icon"} mobileSize="compact" footer={<div className="grid grid-cols-[auto_minmax(0,1fr)] gap-2"><Button type="button" variant="ghost" className="h-12 rounded-xl" onClick={cancelEdit}>Cancel</Button><Button type="button" className="h-12 rounded-xl" disabled={!editingName.trim() || isMutating} onClick={() => void saveEdit()}>{isMutating ? "Saving..." : "Save context"}</Button></div>}>
        <div className="grid gap-4"><div className="relative"><EditIcon className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input value={editingName} onChange={(event) => setEditingName(event.target.value)} className="h-12 rounded-xl pl-10" /></div><TagIconPicker tagName={editingName} value={editingIconKey} onChange={setEditingIconKey} entityLabel="Context" /></div>
      </ResponsiveDialog>
