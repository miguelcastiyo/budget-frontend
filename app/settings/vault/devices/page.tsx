...
export default function VaultDevicesPage() {
  ...
  useEffect(load, [])
  const remove = async () => { if (!removeId) return; setBusy(true); setError(null); try { await apiClient.revokeDevice(removeId); setDevices((current) => current.filter((item) => item.id !== removeId)); setRemoveId(null) } catch { setError("We couldn't remove that device. Try again.") } finally { setBusy(false) } }
  return <div className="min-h-screen bg-background pb-mobile-nav"><header className="sticky top-0 z-40 bg-background/80 backdrop-blur-xl pt-safe-header"><div className="mx-auto flex max-w-lg items-center gap-4 px-5 py-4"><Link href="/settings/vault"><Button variant="ghost" size="icon" className="rounded-full" aria-label="Back to Privacy & Vault"><ArrowLeft className="size-5" /></Button></Link><h1 className="flex-1 text-xl font-bold">Devices</h1></div></header><main className="mx-auto max-w-lg space-y-4 px-5 pt-5"><p className="text-sm text-muted-foreground">Manage the devices with an active Vault session.</p>{error && <p role="alert" className="text-sm text-destructive">{error}</p>}<Card className="divide-y divide-border/70 overflow-hidden rounded-lg border-border/70 py-0 shadow-none">{loading ? <div className="p-5 text-sm text-muted-foreground">Loading devices...</div> : devices.length === 0 ? <div className="p-5 text-sm text-muted-foreground">No active devices found.</div> : devices.map((device) => <div key={device.id} className="flex items-center gap-3 px-4 py-4 sm:px-5"><div className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-secondary/70"><Monitor className="size-5 text-muted-foreground" /></div><div className="min-w-0 flex-1"><p className="truncate font-medium">{device.label || "Unnamed device"}</p><p className="mt-0.5 text-sm text-muted-foreground">{sessionLabel(device)} · {activeLabel(device)}</p>{sessionDate(device.created_at) && <p className="mt-0.5 text-xs text-muted-foreground/80">Session started {sessionDate(device.created_at)}</p>}</div>{!device.is_current && <Button type="button" variant="ghost" className="shrink-0" onClick={() => setRemoveId(device.id)}>Remove</Button>}</div>)}</Card></main><ResponsiveConfirmDialog open={removeId !== null} onOpenChange={(open) => !open && !busy && setRemoveId(null)} title="Remove this device?" description="This device will need to unlock the Vault again before it can view encrypted data." confirmLabel={busy ? "Removing..." : "Remove"} confirmVariant="destructive" confirmDisabled={busy} closeDisabled={busy} onConfirm={() => void remove()} /><BottomNav /></div>
}
