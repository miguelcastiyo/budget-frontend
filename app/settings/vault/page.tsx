...
function SettingsRow({ icon, label, description, meta, onClick, href }: { icon: React.ReactNode; label: string; description?: string; meta?: string; onClick?: () => void; href?: string }) {
  const content = <div className="flex min-h-[68px] items-center gap-3 px-4 py-2.5 text-left sm:gap-4"><div className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-secondary/70">{icon}</div><div className="min-w-0 flex-1"><p className="font-medium leading-tight">{label}</p>{description && <p className="mt-0.5 text-sm leading-snug text-muted-foreground">{description}</p>}</div>{meta && <span className="max-w-[7rem] shrink-0 text-right text-sm leading-tight text-muted-foreground">{meta}</span>}<ChevronRight className="size-5 shrink-0 text-muted-foreground" aria-hidden="true" /></div>
...
export default function VaultSettingsPage() {
  ...
  return <div className="min-h-screen bg-background pb-mobile-nav">
    ...
    <main className="mx-auto max-w-lg space-y-5 px-5 pt-5">
      <p className="text-sm text-muted-foreground">Your financial data is protected by your Vault.</p>
      <section className="space-y-2"><h2 className="px-1 text-sm font-semibold">Vault status</h2><Card className="overflow-hidden rounded-lg border-border/70 py-0 shadow-none"><SettingsRow icon={<LockKeyhole className="size-5 text-muted-foreground" />} label="Vault" description={authority.authority ? "Unlocked on this device" : "Locked"} meta={authority.quickUnlockStatus === "enrolled" ? "Quick Unlock On" : "Quick Unlock Off"} onClick={() => setFlow("unlock")} /></Card></section>
      <section className="space-y-2"><h2 className="px-1 text-sm font-semibold">Security</h2><Card className="divide-y divide-border/70 overflow-hidden rounded-lg border-border/70 py-0 shadow-none"><SettingsRow icon={<KeyRound className="size-5 text-muted-foreground" />} label="Vault passphrase" description="Change your Vault passphrase" onClick={() => setFlow("change-passphrase")} /><SettingsRow icon={<ShieldCheck className="size-5 text-muted-foreground" />} label="Recovery code" meta="Active" onClick={() => setFlow("replace-recovery")} /><SettingsRow icon={<Zap className="size-5 text-muted-foreground" />} label="Quick Unlock" description={authority.quickUnlockCapability === "unsupported" ? "Not available on this device" : "Unlock your Vault faster"} meta={authority.quickUnlockCapability === "supported" ? (authority.quickUnlockStatus === "enrolled" ? "On" : "Off") : undefined} onClick={authority.quickUnlockCapability === "unsupported" ? undefined : () => { setQuickUnlockMessage(""); setQuickUnlockOpen(true) }} /><SettingsRow icon={<Monitor className="size-5 text-muted-foreground" />} label="Devices" description="Manage where your Vault is accessed" meta={deviceCount === null ? undefined : `${deviceCount} ${deviceCount === 1 ? "device" : "devices"}`} href="/settings/vault/devices" /></Card></section>
