...
export default function VaultSettingsPage() {
  ...
  return <div className="min-h-screen bg-background pb-mobile-nav">
    ...
    <main className="mx-auto max-w-lg space-y-6 px-5 pt-5">
      ...
      <section className="space-y-2"><h2 className="px-1 text-sm font-semibold">Quick Unlock</h2><Card className="divide-y divide-border/70 overflow-hidden rounded-lg border-border/70 py-0 shadow-none"><SettingsRow icon={<Zap className="size-5 text-muted-foreground" />} label="Quick Unlock" description={authority.quickUnlockUnavailableReason === "ios_standalone" ? "Open Budget in Safari to use Quick Unlock" : authority.quickUnlockCapability === "unsupported" ? "Not available on this device" : "Use this device to unlock your Vault faster."} meta={authority.quickUnlockCapability === "supported" && authority.quickUnlockStatus === "enrolled" ? "Enabled on this device" : authority.quickUnlockCapability === "supported" ? "Not enabled" : undefined} onClick={authority.quickUnlockCapability === "unsupported" ? undefined : () => { setQuickUnlockMessage(""); setQuickUnlockOpen(true) }} /></Card></section>
