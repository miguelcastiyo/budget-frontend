...
export function exportTransactions(records: TransactionRecord[], options: { tagName?: (id: string | null) => string | null; cardName?: (id: string | null) => string | null; contextName?: (id: string | null) => string | null; createdAt: string; updatedAt: string }): string {
  const header = "date,expense,amount,category,is_split,tag,card,context,created_at,updated_at,notes"
  const quote = (value: string) => `"${value.replace(/"/g, '""')}"`
  const exportExpense = (value: string) => /^[=+\-@]/.test(value) ? `'${value}` : quote(value)
  const rows = [...records].filter((record) => !record.isDeleted).sort((a, b) => b.date.localeCompare(a.date) || a.id.localeCompare(b.id)).map((record) => [record.date, exportExpense(record.expense), formatMoneyCents(record.amountCents), record.category, String(record.isSplit), escapeCsvField(options.tagName?.(record.tagId) ?? null), escapeCsvField(options.cardName?.(record.cardId) ?? null), escapeCsvField(options.contextName?.(record.contextId) ?? null), quote(options.createdAt), quote(options.updatedAt), escapeCsvField(record.notes)].join(","))
  return `${header}\n${rows.join("\n")}\n`
}
