...
import { sameRecurringReference, type RecurringOccurrence } from "./recurring"

export function monthOverview(input: { transactions: TransactionRecord[]; budgets: BudgetSettingsRecord[]; occurrences: RecurringOccurrence[]; projectedRecurring?: TransactionRecord[]; month: string; currentDate: string; recentLimit?: number }) {
  const range = monthDateRange(input.month)
  const recentTransactions = filterTransactions(input.transactions, { from: range.from, to: range.to, sort: "date_asc" }).filter((item) => item.source !== "recurring" || item.date <= input.currentDate)
  const existingRecurring = recentTransactions.filter((item) => item.source === "recurring")
  const projected = (input.projectedRecurring ?? []).filter((item) => !existingRecurring.some((existing) => existing.date === item.date && sameRecurringReference(existing.recurringExpenseId, item.recurringExpenseId)))
  const monthTransactions = [...recentTransactions, ...projected]
  ...
  const recent = [...recentTransactions].sort((a, b) => b.date.localeCompare(a.date) || b.createdSequence - a.createdSequence || a.id.localeCompare(b.id)).slice(0, input.recentLimit ?? 5)
