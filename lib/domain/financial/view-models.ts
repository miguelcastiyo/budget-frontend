...
const sameReferenceId = (left: string, right: string) => left.trim() === right.trim() || left.trim().toLocaleLowerCase() === right.trim().toLocaleLowerCase() || (Number.isFinite(Number(left)) && Number.isFinite(Number(right)) && Number(left) === Number(right))
...
export function transactionsPageFromState(state: RehydratedFinancialState, filter: TransactionFilter = {}, currentDate?: string, includeProjectedRecurring = false): TransactionsPage {
  const visibleTransactions = currentDate ? state.transactions.filter((record) => record.source !== "recurring" || record.date <= currentDate) : state.transactions
  ...
  const page = paginateTransactions(visibleState.transactions, filter)
  const summary = transactionSummary(visibleState.transactions, filter)
  return { items: page.items.map((record) => ({ id: record.id, date: record.date, expense: record.expense, amount: formatMoneyCents(record.amountCents), category: record.category, is_split: record.isSplit, notes: record.notes, source: record.source === "import" ? "import" : "manual", recurring_expense_id: record.recurringExpenseId, tag: tagFromState(state, record.tagId), context: contextFromState(state, record.contextId), card: cardFromState(state, record.cardId), created_at: "", updated_at: "", is_projected: record.id.startsWith("projected:") })), page: page.page, page_size: page.pageSize, total_items: page.totalItems, summary: { total_spent: summary.totalSpent, count: summary.count, avg_transaction: summary.avgTransaction, split_count: summary.splitCount } }
}
export function taxonomyFromState(state: RehydratedFinancialState) { return { tags: state.tags.filter((item) => !item.isDeleted).map((item) => ({ id: item.id, name: item.name, icon_key: item.iconKey })), contexts: state.contexts.filter((item) => !item.isDeleted).map((item) => ({ id: item.id, name: item.name, icon_key: item.iconKey })), cards: state.cards.filter((item) => !item.isDeleted).map((item) => ({ id: item.id, name: item.name, is_favorite: item.isFavorite })) } }
export function tagQuickPicksFromState(state: RehydratedFinancialState, limit = 5): Tag[] { const usage = new Map<string, { count: number; lastUsedAt: string }>(); for (const transaction of state.transactions) { if (transaction.isDeleted || transaction.tagId == null) continue; const current = usage.get(transaction.tagId) ?? { count: 0, lastUsedAt: "" }; current.count++; if (transaction.date > current.lastUsedAt) current.lastUsedAt = transaction.date; usage.set(transaction.tagId, current) }; const tags = state.tags.filter((item) => !item.isDeleted).map((item) => ({ tag: { id: item.id, name: item.name, icon_key: item.iconKey }, usage: usage.get(item.id) ?? { count: 0, lastUsedAt: "" } })); tags.sort((a, b) => b.usage.count - a.usage.count || b.usage.lastUsedAt.localeCompare(a.usage.lastUsedAt) || a.tag.name.localeCompare(b.tag.name, "en-US")); return tags.slice(0, Math.max(0, limit)).map((item) => item.tag) }
