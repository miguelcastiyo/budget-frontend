...
function encryptedSavingsOverview(state: RehydratedFinancialState, month: string, savingsBudgetCents: number): any {
  ...
  const transactionIds = new Set(monthTransactions.map((item) => item.id))
  const transactionDirectedCents = entries.filter((entry) => entry.sourceType === "transaction" && entry.direction === "in" && entry.sourceTransactionId != null && transactionIds.has(entry.sourceTransactionId)).reduce((sum, entry) => sum + entry.amountCents, 0)
...
export function encryptedInsights(state: RehydratedFinancialState, from: string, to: string): any {
  ...
  return { date_from: from, date_to: to, months_in_range: monthlySpendTrend.length, total_spend: result.total, total_transactions: result.count, monthly_spend_trend: monthlySpendTrend, category_breakdown: result.categories.map((item) => ({ category: item.category, spend: item.total, percent_of_total_spend: percent(cents(item.total), totalSpendCents) })), category_budget_vs_actual: result.categories.map((item) => { const budgetAmount = budgetTotals[item.category as keyof typeof budgetTotals]; return { category: item.category, budget_amount: formatMoneyCents(budgetAmount), actual_spend: item.total, percent_used: percent(cents(item.total), budgetAmount) } }), tag_breakdown: [...tagTotals.entries()].map(([tag_id, spend]) => ({ tag_id, tag_name: state.tags.find((tag) => tag.id === tag_id)?.name ?? "", icon_key: state.tags.find((tag) => tag.id === tag_id)?.iconKey ?? null, spend: formatMoneyCents(spend), percent_of_total_spend: percent(spend, totalSpendCents) })), day_of_week_spend: weekday, largest_transactions: result.largest.map((item) => ({ transaction_id: item.id, date: item.date, expense: item.expense, amount: formatMoneyCents(item.amountCents), category: item.category, is_split: item.isSplit, notes: item.notes, tag: { id: item.tagId ?? "", name: state.tags.find((tag) => tag.id === item.tagId)?.name ?? "", icon_key: state.tags.find((tag) => tag.id === item.tagId)?.iconKey ?? null }, card_name: state.cards.find((card) => card.id === item.cardId)?.name ?? null })), recurring_vs_variable: { recurring: result.recurring, variable: result.variable, recurring_percent: percent(cents(result.recurring), totalSpendCents), variable_percent: percent(cents(result.variable), totalSpendCents) } }
}
